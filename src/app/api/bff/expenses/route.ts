...
import { checkRateLimit, API_RATE_LIMIT } from "@/lib/middleware/rate-limit";
...
export async function GET(req: NextRequest) {
  ...
  const rl = checkRateLimit(req, "api", API_RATE_LIMIT);
  if (!rl.success) {
  ...
  try {
    const data = await internalApi.get<PaginatedResponse<ExpenseSummary>>(
...
export async function POST(req: NextRequest) {
  ...
  try {
    const data = await internalApi.post("/expenses", parsed.data);
